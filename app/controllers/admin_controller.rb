class AdminController < ApplicationController
  ...
  def index
    ...
    @recentEvents = Behavior.order("event_date DESC", "event_time DESC").where(event_date: 1.week.ago..Date.today).limit(10)
    ...
    @highFliers = Behavior.select(:student_id, 'COUNT(*) as count').group(:student_id).where('student_id IS NOT NULL').order('count desc')
